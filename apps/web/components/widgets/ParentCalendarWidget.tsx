...
export default function ParentCalendarWidget({
...
}: ParentCalendarWidgetProps): React.JSX.Element {
  ...
  const renderCalendarDays = (): React.JSX.Element[] => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      const isHighlighted = isInitialMonth && highlightedDays.includes(day);
    ...
    <div className="w-full max-w-5xl bg-[#2a2614] dark:bg-[#1C1A0D] rounded-xl border border-white/5 shadow-2xl overflow-hidden">
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="lg:w-5/12 border-b lg:border-b-0 lg:border-r border-white/5 p-6 bg-[#232010]">
          <div className="flex flex-col gap-4">
            ...
            <div className="flex items-center justify-between px-2 mb-2">
              <button onClick={goToPrevMonth} className="hover:bg-white/5 p-2 rounded-full transition-colors">
                <span className="material-symbols-outlined text-white">chevron_left</span>
              </button>
              <p className="text-white text-lg font-bold leading-tight">{monthName} {currentYear}</p>
              <button onClick={goToNextMonth} className="hover:bg-white/5 p-2 rounded-full transition-colors">
        ...
        <div className="lg:w-7/12 p-6 flex flex-col gap-6">
          {isInitialMonth && events.length > 0 ? (
            ...
            <div className="flex flex-col items-center justify-center py-8 text-center">
              ...
              <p className="text-slate-400 text-sm">No events scheduled for {monthName} {currentYear}.</p>
            </div>
          )}
