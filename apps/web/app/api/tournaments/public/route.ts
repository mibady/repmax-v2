...
export async function GET(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    let query = supabase
      .from("tournaments")
      .select(
        "id, name, description, start_date, end_date, location, teams_capacity, teams_registered, entry_fee_cents, registration_deadline, is_public, event_tier, event_type, zone, status"
      )
      .eq("is_public", true)
    ...
    let rostersByTournament: Record<string, { player_name: string; position: string | null; class_year: number | null }[]> = {};
    ...
    if (tournamentIds.length > 0) {
      ...
      if (regData) {
        ...
        // Get roster entries to show top prospects attending
        ...
        if (regIds.length > 0) {
          ...
          if (rosterData) {
            // Map registration_id -> tournament_id
            ...
            for (const roster of rosterData) {
              const tournamentId = regToTournament[roster.registration_id];
              if (tournamentId) {
                if (!rostersByTournament[tournamentId]) {
                  rostersByTournament[tournamentId] = [];
                }
                rostersByTournament[tournamentId].push({
    ...
    const enriched = (tournaments || []).map(
      (t: { id: string; teams_registered: number }) => ({
        ...
        prospects: rostersByTournament[t.id] || [],
      })
    );
